...
class money_hr(object):  # 計算時薪

   def __inti__(self, money_mounth, hr, day):
       
       self.money_mounth = money_mounth
       self.hr = hr
       self.day = day

   def __str__(self):
       """
       時薪: 月薪 (money_mounth) / 小時 (hr) * 天數 (day)
       """
       return '時薪' + str(self.money_mounth / (self.hr *self.day))
   
   def __sub__(self, other):  # 兩份工作行情的比較
       return str(self.money_mounth / (self.hr *self.day) \
                        - other.money_mounth / (other.hr *other.day))
